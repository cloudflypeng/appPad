...
function FixedTopBar({ title, description, containerRef, onRefresh }: FixedTopBarProps): React.JSX.Element {
  ...
  return (
    <header
      className="fixed top-0 z-40 flex h-16 w-full items-center justify-between border-b bg-background/95 px-6 backdrop-blur md:px-8"
    ...
    >
      <div className="flex items-center">
        ...
        <div className="ml-4">
          ...
          {description ? <p className="text-xs text-muted-foreground leading-tight">{description}</p> : null}
        </div>
      </div>
      {onRefresh ? (
